    ...
    private fun setupUI() {
        ...
        appAdapter = ParentalAppAdapter(
            ...
            onDeleteClicked = { app ->
                val fid = currentConfig?.familyId ?: return@ParentalAppAdapter
                lifecycleScope.launch(Dispatchers.IO) {
                    syncEngine.removeAppRestriction(fid, app.packageName)
                }
            }
        )
        ...
        binding.switchGlobalRestrictions.setOnCheckedChangeListener { _, isChecked ->
            val fid = currentConfig?.familyId ?: return@setOnCheckedChangeListener
            lifecycleScope.launch(Dispatchers.IO) {
                syncEngine.writeGlobalEnabled(fid, isChecked)
            }
        }
            ...
            withContext(Dispatchers.Main) {
                ...
                if (config == null || config.role == null) {
                    ...
                    if (config.isPaired) {
                        ...
                        // returning child only ever picks up a parent's change via the 15-min
                        // SyncWorker floor (there's no FCM push implemented yet either).
                        syncOnAppOpen()
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            ...
            withContext(Dispatchers.Main) {
                ...
                if (result.isSuccess) {
                    ...
                    Toast.makeText(
                        this@ParentalControlActivity,
                        result.exceptionOrNull()?.localizedMessage ?: getString(R.string.error_auth_failed),
                        Toast.LENGTH_LONG
                    ).show()
                }
            }
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            ...
            withContext(Dispatchers.Main) {
                ...
                if (familyId != null) {
                    ...
                    Toast.makeText(
                        this@ParentalControlActivity,
                        claimResult.exceptionOrNull()?.localizedMessage ?: getString(R.string.error_invalid_code),
                        Toast.LENGTH_LONG
                    ).show()
                }
            }
                ...
                if (requestDoc != null) {
                    ...
                    binding.btnApproveRequest.setOnClickListener {
                        ...
                        lifecycleScope.launch(Dispatchers.IO) {
                            ...
                            syncEngine.writeAppRestriction(familyId, pkg, appName, true, newAllowance)
                        }
                    }
                    ...
                    binding.btnDenyRequest.setOnClickListener {
                        ...
                        binding.btnDenyRequest.isEnabled = false
                        lifecycleScope.launch(Dispatchers.IO) {
                            requestDoc.reference.update("status", "DENIED")
                        }
                    }
                } else {
